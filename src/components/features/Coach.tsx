...
const Coach: React.FC = () => {
  ...
  const totalCoaches = rows.flat().length;
  const [hovered, setHovered] = useState(
    new Array(totalCoaches).fill(false)
  );
  let offset = 0;
                  ...
                  <div className="swiper--container">
                    {row.map((coach, index) => {
                      ...
                      return (
                        <SwiperSlide
                          key={globalIndex}
                          className={index === row.length - 1 ? "last" : ""}
                        >
                          <div
                            className="card"
                            onMouseEnter={() =>
                            ...
                            onMouseLeave={() =>
                              setHovered(prev => {
                                const copy = [...prev];
                                copy[globalIndex] = false;
                                return copy;
                              })
                            }
                            style={{
                              backgroundImage: `url(${coach.background})`,
                              backgroundSize: hovered[globalIndex]
                                ? "105%"
                                : "cover",
